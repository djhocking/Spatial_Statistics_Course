...
Euclid.Dist <- data.frame(c(dist.NYC.LA, dist.NYC.Sea, dist.LA.Sea), row.names = c('dist.NYC.LA', 'dist.NYC.Sea', 'dist.LA.Sea'))
...
```{r}
Geo.Dist <- data.frame(c(gdist.NYC.LA, gdist.NYC.Sea, gdist.LA.Sea), row.names = c('gdist.NYC.LA', 'gdist.NYC.Sea', 'gdist.LA.Sea'))
names(Geo.Dist) <- 'Km'
...
```{r}
# Asssume earth sphere with diameter = 6378 km
# Calculate chord distance through sphere (3D Pythagorean Theorem)
r.earth <- 6378
...
```{r}
Chord.Dist <- data.frame(c(cdist.NYC.LA, cdist.NYC.Sea, cdist.LA.Sea), row.names = c('cdist.NYC.LA', 'cdist.NYC.Sea', 'cdist.LA.Sea'))
names(Chord.Dist) <- 'Km'
...
map("world", proj='albers', par=c(30,40), xlim=c(-130, -20), ylim=c(25,85)); title("albers") # equal area
map("world", xlim=c(-130, -20),ylim=c(25,85), proj='bonne', param=35); title("bonne") # equal spaced parallels, equal area projection
map("world", xlim=c(-130, -20),ylim=c(25,85), proj='lambert', par=c(30,40)); title("lambert") # conformal
```
